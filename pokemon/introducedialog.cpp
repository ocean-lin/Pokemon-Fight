...
void IntroduceDialog::on_pushButton_clicked()
{
    this->close();
    t->stop();
    emit show_choosedialog();
}

void IntroduceDialog::update_pokemon_move()
{
    f = (f + 1) % 4;
...
void IntroduceDialog::receivemake_IntroduceDialog(Pokemon *x, int y, int z)
{
    xi = x;
    n = y;
    m = z;
...
void IntroduceDialog::on_pushButton_2_clicked()
{
    this->close();
    t->stop();
    emit sendpokemon(xi,n);
    emit choosepokemon(m);
}
